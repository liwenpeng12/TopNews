...
public class TopFragment extends LazyLoadFragment  {
    ...
    private View view;
    private GDBean gdBean;
    private List<GDBean> savedlist;
    ...
    protected int setContentView() {
        if (NetStatusUtil.isConnected(getActivity())) {
            ...
            if (null == MyApplication.getInstances().getDaoSession().getGDBeanDao().queryBuilder()
                    .build().list().toString()) {
                //没有缓存
    ...
    protected void lazyLoad() {
        ...
        if (NetStatusUtil.isConnected(getActivity())) {
            ...
            GDBeanDao gdBeanDao = MyApplication.getInstances().getDaoSession().getGDBeanDao();
            savedlist = gdBeanDao.queryBuilder()
                    .build().list();
            //没有网络
            //1.查看是否有缓存
       ...
       //         view = inflater.inflate(R.layout.fragment_top, null, false);
                ...
                List<GDBean> dataBeanList = savedlist;
